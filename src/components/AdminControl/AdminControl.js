...
import Donut from "../MyDashBoard/Donut";
import Areachart from "../MyDashBoard/Areachart";
import DataTable from "../MyDashBoard/DataTable";
import ViewCount from "./ViewCount";
import AdminControlConfig from "./AdminControlConfig";

const AdminControl = ({menuOpen, setMenuOpen, expiryTimestamp}) => {
        ...
        <div className="d-flex align-items-start justify-content-between  gap-3 dashboardItem">
          ...
          <div className="d-flex align-items-center flex-wrap justify-content-end dashboardRight">
            <ul className="list-unstyled d-flex align-items-center mb-4">
              {menu.map((item) => (
                <FilterableTab tabIcon={item.tabIcon} title={item.title} active={selected === item.id}
                               setSelected={setSelected} id={item.id} key={item.id}/>
              ))}
            </ul>
